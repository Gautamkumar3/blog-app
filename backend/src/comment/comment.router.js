  ...
  try {
    if (data.id === unique) {
      ...
      res
        .status(401)
        .send(
          "You are not the owner of this Comment so you can't delete this Comment"
        );
    }
  } catch (e) {
...
app.patch("/:id", Authmiddleware, async (req, res) => {
  ...
  try {
    if (data.id === unique) {
      let afterUpdate = await Comment.findByIdAndUpdate(
        id,
      ...
      res
        .status(401)
        .send("You are not the owner of this Comment so you can't update");
    }
  } catch (e) {
    res.status(401).send(e.message);
  }
